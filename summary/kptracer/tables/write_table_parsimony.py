...
if __name__ == "__main__":
    ...
    sys.stdout.write("\\caption[SH parsimony scores for KP-Tracer tumors]")
    sys.stdout.write("{\\textbf{SH parsimony scores for KP-Tracer tumors.}}")
    ...
    for data in [3, 2, 1]:
        if data == 1:
            ...
            df = pandas.read_csv("../csvs/3515_Lkb1_T1_Fam_parsimony_score.csv",
                                 na_values='NA', keep_default_na=False)
        elif data == 2:
            name = "3724\\_NT\\_All"
            df = pandas.read_csv("../csvs/3724_NT_All_parsimony_score.csv",
                                 na_values='NA', keep_default_na=False)
        elif data == 3:
            name = "3513\\_NT\\_T1\\_Fam"
            df = pandas.read_csv("../csvs/3513_NT_T1_Fam_parsimony_score.csv",
                                 na_values='NA', keep_default_na=False)
        else:
