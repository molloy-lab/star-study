...
for folder in folders:
    ...
    contract_path = os.path.join(result_dir, folder, 'star_cdp_one_sol_deduplicate_contract_sh.tre')
    ...
    deduplicate_one_sol_path = os.path.join(result_dir, folder, 'star_cdp_rand_sol_deduplicate.tre')
    contract_path = os.path.join(result_dir, folder, 'star_cdp_rand_sol_deduplicate_contract_sh.tre')

    score_res = sp.run(['python3', comp_exe, '-t1', deduplicate_one_sol_path, '-t2', deduplicate_one_sol_path, '-c1','1', '-c2', '1', '-m', cmat_path, '--extract_tree2', '1', '-t2p', contract_path], capture_output=True, text=True)  

    deduplicate_one_sol_path = os.path.join(result_dir, folder, 'star_cdp_sc_deduplicate.tre')
    contract_path = os.path.join(result_dir, folder, 'star_cdp_sc_deduplicate_contract_sh.tre')
    score_res = sp.run(['python3', comp_exe, '-t1', deduplicate_one_sol_path, '-t2', deduplicate_one_sol_path, '-c1','1', '-c2', '1', '-m', cmat_path, '--extract_tree2', '1', '-t2p', contract_path], capture_output=True, text=True)  