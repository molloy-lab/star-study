...
def write_star_cdp_sbatch(exe: str, cmat: str, priors: str, sbatch_file: str, search_space: str):
    ...
    rows_to_wrtie = ['#!/bin/bash', 
                    ...
                    ' '.join([time,'-v','-o','consensus_star_cdp_usage.log',exe,'-i',cmat, '-m', priors, '-x OUTG','-t',search_space, '-consensus','--output', 'star_cdp', '-o','consensus_star_cdp','-XOUTG',\
...
def main():
    ...
    for folder in folders:
        ...
        for rep in reps:
            ...
            star_cdp_sbatch = os.path.join(cur_res_rep_path, 'run_star_cdp_consensus.sbatch')
