...
def main():
    ...
    for folder in folders:
        ...
        for rep in reps:
            ...
            score_path = os.path.join(cur_res_rep_path, 'Rand-RFSH.csv')

            true_tree_path = os.path.join(os.path.join(cur_data_path, rep), 'contracted_true_tree.newick')


            star_cdp_tree_path = os.path.join(cur_res_rep_path, 'star_cdp_random_sol_trees.tre')
            ...
            , '-t1', true_tree_path, '-t2', star_cdp_tree_path, '-c1','0', '-c2', '1', '-m', cmat_path ,'-r', '0'], capture_output=True, text=True)
