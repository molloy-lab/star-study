...
def submit_laml_sbacth(sbatch: str, data_prefix: str, res_dir:str):
    ...
    if os.path.exists(os.path.join(res_dir,'nni_tree_deduplicate.tre')):
...
def main():
    ...
    for folder in folders:
        ...
        # if os.path.exists(os.path.join(cur_res_path, "nni_tree_deduplicate.tre")):
        if os.path.exists(os.path.join(cur_res_path, "nni_tree_deduplicate_contract_sh.tre")):
            write_laml_sbatch(cmat_path, priors_path, laml_sbatch, os.path.join(cur_res_path, "nni_tree_deduplicate.tre"), os.path.join(cur_res_path,'one_sol_branch_resolve_poly'))
            # write_laml_sbatch(cmat_path, priors_path, laml_sbatch, os.path.join(cur_res_path, "nni_tree_deduplicate_contract_sh.tre"), os.path.join(cur_res_path,'one_sol_sh_branch_resolve_poly'))
