...
def compute_equivalence_classes_up_to_exact(df_character_matrix):
    df_character_matrix = df_character_matrix.copy()
    df_character_matrix = df_character_matrix.astype(int)
    ...
    for cell in df_binary.index:
        ...
        for eq_class in eq_class_list:
            leader_cell = eq_class[0]
            if (one_set_dict[leader_cell] == (one_set_dict[cell]) and
                zero_set_dict[leader_cell] == (zero_set_dict[cell])):
                eq_class.append(cell)
                flag = 1
                break
            elif (one_set_dict[cell] == (one_set_dict[leader_cell]) and
                  zero_set_dict[cell] ==(zero_set_dict[leader_cell])):
                eq_class.appendleft(cell)
                flag = 1
                break
