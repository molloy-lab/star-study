...
def write_star_cdp_sbatch(exe: str, cmat: str, priors: str, sbatch_file: str, search_space: str):
    ...
    rows_to_wrtie = ['#!/bin/bash', 
                    ...
                    ' '.join([time,'-v','-o','star_cdp_usage.log',exe,'-i',cmat, '-m', priors, '-x 0','-t',search_space, '-consensus','-rand 1','--output', 'star_cdp','-nosupp',\
                    '&>','star_cdp.log','2>&1'])
                    ]
...
def main():
    ...
    for folder in folders:
        ...
        cmat_path = os.path.join(cur_res_path, folder + '_deduplicated_character_matrix_with_outg.csv')
