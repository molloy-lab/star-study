...
def main():
    ...
    for folder in folders:
        ...
        one_sol_path = os.path.join(result_dir,folder, 'deduplicated_star_cdp_one_sol_trees.nwk')
            
        one_sh_path = os.path.join(result_dir, folder, 'deduplicated_star_cdp_one_sol_sh_trees.nwk')

        rand_sol_path = os.path.join(result_dir,folder, 'deduplicated_star_cdp_rand_sol_trees.nwk')
            
        rand_sh_path = os.path.join(result_dir, folder, 'deduplicated_star_cdp_rand_sol_sh_trees.nwk')


        sc_path = os.path.join(result_dir, folder, 'deduplicated_star_cdp_sc_trees.nwk')
            
        sc_sh_path = os.path.join(result_dir,folder, 'deduplicated_star_cdp_sc_sh_trees.nwk')


        trees = [one_sh_path,sc_path, sc_sh_path, rand_sh_path]

        
        for sh_tree in trees:
            method_name = sh_tree[sh_tree.find('cdp_') + 4:-10]
            print(method_name)
