...
def submit_laml_sbacth(sbatch: str, data_prefix: str, res_dir:str):
    job_name = "--job-name=" + "e.ilp-dedup." + data_prefix
    output =  "--output="+ "e.ilp-dedup." + data_prefix + ".%j.out"
    err = "--error=" + "e.ilp-dedup." + data_prefix + ".%j.err"
    os.chdir(res_dir)
...
def main():
    ...
    result_dir = '/fs/cbcb-lab/ekmolloy/jdai123/star-study/F_bio_result_deduplicate/startle_ilp'
    ...
    for folder in folders:
        ...
        if os.path.exists(os.path.join(cur_res_path, "output", 'startle_tree.newick')):
            # write_laml_sbatch(cmat_path, priors_path, laml_sbatch, os.path.join(cur_res_path, "removed_outg_star_cdp_sc_strict_consensus.tre"), os.path.join(cur_res_path,'star_cdp_sc_branch'))
            ...
            write_laml_sbatch(cmat_path, priors_path, laml_sbatch, os.path.join(cur_res_path, 'one_sol_branch_trees.nwk'), os.path.join(cur_res_path,'one_sol_branch_resolve_poly'))
