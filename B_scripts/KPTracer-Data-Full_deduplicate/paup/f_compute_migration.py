...
def process_method(folder, result_dir, data_dir, startle_exe, index_for_dp_list, taxon2order, 
    ...
    cur_tree_path = os.path.join(result_dir, folder, f'deduplicated_paup_{method_name}_trees.nwk')
    ...
    cur_sh_tree_path = os.path.join(result_dir, folder, f'deduplicated_paup_{method_name}_sh_trees.nwk')
...
for folder in folders:
    ...
    data = data + [folder] * 5
    metric = metric + ['migration', 'reseeding', 'migration SH Contract', 'reseeding SH Contract', 'parsimony score']
    ...
    cur_paup_res = process_method(folder, result_dir, data_dir, startle_exe, index_for_dp_list, taxon2order, 
                   index_to_site_map, name_to_index_map, 'one_sol')

    paup_res  = paup_res + cur_paup_res

    cur_paup_sc_res = process_method(folder, result_dir, data_dir, startle_exe, index_for_dp_list, taxon2order, 
                   index_to_site_map, name_to_index_map, 'sc')


    paup_sc_res  = paup_sc_res + cur_paup_sc_res
