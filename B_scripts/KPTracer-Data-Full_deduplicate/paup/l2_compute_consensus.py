...
def write_paup_sbatch(curr_dir : str, paup_exe: str, paup_nex_file:str, sbatch_file:str):
    ...
    rows_to_wrtie = ['#!/bin/bash', 
                    ...
                    ' '.join([time, '-v', '-o', 'deduplicate_paup_consensus_usage.log', paup_exe, paup_nex_file, '&>', 'deduplicate_consensus_paup.log', '2>&1'])
                    ]
...
def main():
    ...
    for folder in folders:
        ...
        paup_nex_file = os.path.join(cur_res_path, 'deduplicate_paup_consensus.nex')
        # paup_sbatch = os.path.join(cur_res_path, 'run_consensus_paup.sbatch')
        paup_sbatch = os.path.join(cur_res_path, 'deduplicate_run_consensus_paup.sbatch')
