...
def main():
    ...
    # '3724_NT_All']
    # folders = [
    # '3515_Lkb1_T1_Fam']   done

    folders = [
    '3513_NT_T1_Fam']  


    methods = ['paup_one_sol', 'paup_sc', 'star_cdp_one_sol', 'star_cdp_rand_sol', 'star_cdp_sc', 'startle_nni', 'startle_nni_pub', 'startle_nni_python', 'startle_ilp']


    for folder in folders:
        
        for method in methods:
            sh_contract_folders = os.path.join(result_dir, method, folder)
            ...
            one_sol_path = os.path.join(result_dir, method, folder, 'one_sol_trees.nwk')

            if not os.path.exists(one_sol_path):
                one_sol_path = os.path.join(result_dir, method, folder, 'one_sol_24hrs_best_trees.nwk')
            
            one_sh_path = os.path.join(result_dir, method, folder, 'one_sol_sh_trees.nwk')
            # trees = [one_sol_path, one_sh_path]

            trees = [one_sh_path]

        
            for sh_tree in trees:
                method_name = sh_tree[:-10]
                print(method_name)
