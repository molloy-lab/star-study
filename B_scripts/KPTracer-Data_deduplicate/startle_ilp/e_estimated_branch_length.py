...
def write_laml_sbatch(cmat: str, priors: str, sbatch_file: str, start_tree: str, prefix:str):
    ...
    rows_to_wrtie = ['#!/bin/bash', 
                    ...
                    ' '.join([time,'-v','-o','one_sol_sh_branch_usage.log','run_laml','-c',cmat, '-p', priors,'-t',start_tree, '-o', prefix ,'-v','--nInitials','1','--timescale','6','&>','one_sol_sh_branch.log','2>&1'])
                    ]
...
def main():
    ...
    for folder in folders:
        ...
        if os.path.exists(os.path.join(cur_res_path, "output", 'startle_tree.newick')):
            ...
            write_laml_sbatch(cmat_path, priors_path, laml_sbatch, os.path.join(cur_res_path, 'one_sol_sh.tre'), os.path.join(cur_res_path,'one_sol_sh_branch'))
