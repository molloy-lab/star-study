...
def write_nj_sbatch(curr_dir: str, nj_exe: str, cmat: str, sbatch_file: str):
    ...
    nj_tree_file = os.path.join(curr_dir, "deduplicated_nj.newick")
    time="/usr/bin/time"
...
def submit_nj_sbatch(data_prefix: str, nj_sbatch: str, res_dir: str):
    ...
    err = "--error=" + "a.nj." + data_prefix + ".%j.err"
    os.chdir(res_dir)
    if not os.path.exists(os.path.join(res_dir, 'deduplicated_nj.newick')):
...
def main():
    ...
    for folder in folders:
        ...
        nj_sbatch = os.path.join(cur_res_path, 'run_deduplicated_nj.sbatch')
        data_prefix = folder 
        print(data_prefix)
        # if not os.path.exists(os.path.join(cur_res_path, "nj.newick")) or True:
        if not os.path.exists(os.path.join(cur_res_path, "deduplicated_nj.newick")):
